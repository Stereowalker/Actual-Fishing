...
import com.stereowalker.actualfishing.ActualFishing;
...
public class AttributeTemplate {
    ...
    public void realize(BiConsumer<Holder<Attribute>, AttributeModifier> actions, String slot) {
        realize(actions, ActualFishing.CURIO_MODIFIERS.getOrDefault(slot, VersionHelper.toLoc("tiered","curio_rings")));
    }
...
//        Holder<Attribute> key = RegistryHelper.getAttribute(new ResourceLocation(attributeTypeID));
        if(key == null || key.isEmpty()) {
            ActualFishing.LOGGER.warn(String.format("%s was referenced as an attribute type, but it does not exist! A data file in /tiered/item_attributes/ has an invalid type property.", attributeTypeID));
        } else {
